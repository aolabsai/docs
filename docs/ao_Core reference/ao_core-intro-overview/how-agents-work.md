...
the main method! Given a binary string as input in the proper shape (e.g. 28x28=784 for downsampled MNIST), .next\_state advances the agent to the next state and returns an output. Labels, instincts, and other learning and control settings can be applied.
