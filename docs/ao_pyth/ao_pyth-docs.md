...
#### To initalise an Agent either of the 2 options below:
...
```python
agent = ao.Agent(kennel_id=<unique kennel id>, 
                 api_key=api_key)
```
...
#### To invoke/ Train an Agent to get its output, use its next_state method:
```python
